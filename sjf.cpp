...
int findMinArrivalTime(struct SJF a[],int n){
	int ind=-1;
	int mini=1000;
	for(int i=0;i<n;i++)
	{
		if(a[i].at<=mini&&a[i].completed==0)
		{
			mini=a[i].at;
			ind=i;
		}
...
int main()
{
	...
	for(int i=0;i<n;i++)
	{
		if(i==0)
		{
			index=findMinArrivalTime(a,n);
			completedTime=a[index].at;
		}
		else
		{
			index=findMinBurstTime(a,n,completedTime);
		}
		if(index==-1)
		{
			index=findMinArrivalTime(a,n);
	...
	cout<<"average wating time is "<<avgwt<<endl<<"average turnaround time is "<<avgtat;
}
